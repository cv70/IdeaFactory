...
3. 再读技术设计，理解 `run -> agent session -> graph mutation -> projection` 的内核语义和状态流转。
...
当前技术实现方向不再把 runtime 只理解成一个抽象状态机，而是参考 `backend/deep` 的模式，把 `Idea Factory` 落成一套 `agent-driven graph runtime`：

- 一个 `MainAgent` 负责解释 `workspace` 目标、读取当前 graph、决定下一步探索与收敛动作，并直接驱动 graph 生长。
- 程序侧只保留调度、最小结构校验、持久化、mutation 广播与恢复，不在后端内核里固化 graph 阶段策略。
- 系统内部持续平衡 `发散/收敛`、`研究/产出`、`激进/稳健` 三组行为轴，用户默认不直接调这些轴。
- 所有执行都通过受控工具面完成，包括 graph 追加、研究、文件访问和运行时工具能力。
- `graph`、`projection`、`decision`、`evidence` 和 `artifact` 是 agent 运行后的持久化结果层，而不是模型上下文里的隐式状态。
- `intervention` 的实现语义是高层方向治理，系统通过更新 `MainAgent` 上下文与关注重点来改变后续探索走向。
