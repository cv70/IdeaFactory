...
func TestBuildInitialPlanUsesModelSteps(t *testing.T) {
	domain := NewExplorationDomain(nil, fakePlanningModel{})
	session := createWorkspace(CreateWorkspaceReq{
		Topic:      "AI education",
		OutputGoal: "Research directions",
	})
	_, steps := buildInitialPlan(domain, session, "run-1", time.Now())
	if len(steps) != 3 {
		t.Fatalf("expected 3 steps, got %d", len(steps))
	}
	if steps[0].Desc != "scan evidence" {
		t.Fatalf("unexpected first step: %s", steps[0].Desc)
	}
	if steps[1].Desc != "update direction graph" {
		t.Fatalf("unexpected second step: %s", steps[1].Desc)
	}
	if steps[2].Desc != "package top idea" {
		t.Fatalf("unexpected third step: %s", steps[2].Desc)
	}
}
